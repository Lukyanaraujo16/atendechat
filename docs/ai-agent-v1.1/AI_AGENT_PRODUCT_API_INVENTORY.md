...
- **Fase 2.8B.1 concluída (hardening crítico credenciais):** DELETE legado bloqueia uso por `AiAgent` e `AiKnowledgeEmbeddingSettings`; PUT legado rejeita propriedade `enabled` (autoridade Product enable/disable). GET list legado permanece para KB. Defesa em profundidade nos services.
- **Fase 2.8B.2 concluída (bloqueio seletivo mutações comerciais):** mutações HTTP comerciais legadas (`POST/PUT/DELETE /ai-agents*`, profile, knowledge-agent, simulator legado, shadow suggestion review, `POST/PUT/DELETE/test` credenciais) respondem **410** com `ERR_AI_AGENT_LEGACY_MUTATION_DISABLED` / `ERR_AI_PROVIDER_CREDENTIAL_LEGACY_MUTATION_DISABLED`. Rotas **permanecem registradas**; controllers/services **não removidos**. Preservados: `GET /ai-agents` (Console), `GET /ai-provider-credentials` (KB), Analytics/Shadow FC/evaluations técnicos, Product API, Runtime. Remoção física = **2.8B.3 (pendente)**. Knowledge Base Product permanece backlog separado.