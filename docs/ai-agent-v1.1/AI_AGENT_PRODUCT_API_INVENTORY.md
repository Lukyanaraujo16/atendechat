...
- **Fase 2.8B.1 concluída (hardening crítico credenciais):** DELETE legado bloqueia uso por `AiAgent` e `AiKnowledgeEmbeddingSettings`; PUT legado rejeita propriedade `enabled` (autoridade Product enable/disable). GET list legado permanece para KB. Endpoints legados **não removidos**; bloqueio amplo de mutações comerciais = **2.8B.2 (pendente)**. Knowledge Base Product permanece backlog separado.