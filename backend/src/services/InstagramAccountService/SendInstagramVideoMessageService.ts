...
import {
  ...
  isInstagramVideoFormatError,
  mapInstagramOutboundSendError,
  sendInstagramDirectVideoMessage
...
const SendInstagramVideoMessageService = async ({
  ticket,
  media,
  body,
  companyId
  ...
  const publicVideoUrl = buildInstagramPublicMediaUrl(savedFile.relativePath);
  const caption = formatBody(body?.trim() || "", ticket.contact);
  const bodyToSave = caption || "Vídeo";
  ...
  logger.info(
    ...
    "[InstagramVideoOutbound] sending"
  );

  try {
    const sendResult = await sendInstagramDirectVideoMessage(
      businessId,
      recipientId,
      publicVideoUrl,
      accessToken
    ...
    if (mapped.appError.message === "ERR_INSTAGRAM_SEND_FAILED") {
      throw new AppError(
        "ERR_INSTAGRAM_VIDEO_SEND_FAILED",
        mapped.appError.statusCode,
        "Não foi possível enviar vídeo pelo Instagram."
      );
    }
    ...
    logger.warn(
      ...
      "[InstagramVideoOutbound] failed"
    );
    throw mapped.appError;
  }
};

export default SendInstagramVideoMessageService;
